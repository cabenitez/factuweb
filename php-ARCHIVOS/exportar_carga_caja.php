...
include("conexion.php");
	...
	$consulta ="select descripcion,n_sucursal, n_factura, razon_social, sum(round(total_sin_impuesto,2)), sum(round(iva,2)), sum(round(otros_impuestos,2)), sum(round(total_general,2)), talonario , zona, cod_talonario 
				   FROM	(
						...
						(factura_vta_no_cliente.num_talonario)talonario, factura_vta_no_cliente.cod_zona as zona, factura_vta_no_cliente.cod_talonario
					...
					UNION ALL
						...
						(factura_vta.num_talonario)as talonario, factura_vta.cod_zona as zona, factura_vta.cod_talonario
...
$pdf2->Cell(0,3,'REPARTIDOR: '.$repartidor.' - '.$nombre_fletero.'                           USUARIO: '.$nombre_usuario_carga.'                           ZONA: '.$zona_carga,0,1);
...
do{ 					// obtengo los resultados 
		...
		$total_carga_factura = $total_carga_factura;
...
if(empty($destino)){
	...
	$pdf2->Output('pdf/'.$usuario_sesion.'2.pdf','F');   // guarda en el server
}
