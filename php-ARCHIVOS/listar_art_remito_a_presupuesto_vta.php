	...
	if($pag_filas > 0){ //Si es una Factura a CLIENTES LEVANTA LOS ARTICULOS DEL REMITO ************************************************
		...
		echo "<table width='100%'  border='0'cellspacing='1' cellpadding='0'>";
			...
			while($registro = mysql_fetch_array($pag_result)){ 	// obtengo los resultados 
					...
					//--------------------------AGREGAR ARTICULO A TABLA DE PRESUPUESTO DETALLE TEMPORAL---------------------------------//				
					$consulta = "SELECT count(linea) FROM presupuesto_vta_tmp where usuario = '$usuario_presu'"; // obtiene el numero de sucursal
					$result = mysql_query($consulta);            // hace la consulta
					$registro = mysql_fetch_row($result);        // toma el registro
					$num_iter = $registro[0]; 	// OK
	
					$consulta = "SELECT * FROM presupuesto_vta_tmp where usuario = '$usuario_presu' and cod_prod = $codigo";		// obtiene la cantidad de articulos
					$result = mysql_query($consulta);            
					$nfilas = mysql_num_rows ($result);          		//indica la cantidad de resultados
					if ($nfilas == 0){     						 		// si ya existe el articulo
						if($num_iter < $max_iter){																	// verifica que las lineas sean menor al max	
							$consulta = "call alta_presupuesto_vta_tmp('$usuario_presu', $codigo,'$descripcion',$cantidad,$precio,$bonif,$importe,$fila,$tasa_iva)"; // llama al procedimiento almacecnado
							$result = mysql_query($consulta);        
							//echo $consulta; 
						}	
					}	
