...
if($razon){ 
	...
	*/
	...
	if ($nfilas > 0){												// si existen facturas obtiene el mayor
		$num_presupuesto = $registro[0]+1;
	}
	*/
	$consulta = "SELECT n_presupuesto FROM presupuesto_vta where cod_talonario = '$codigo_tal'"; 				// verifica si se han hecho facturas a un cliente
	$result = mysql_query($consulta);            					
	$nfilas = mysql_num_rows ($result);          					
	if ($nfilas > 0){												// si existen facturas obtiene el mayor
		$consulta = "SELECT max(n_presupuesto) FROM presupuesto_vta where cod_talonario = '$codigo_tal'"; 
		$result = mysql_query($consulta);            
		$registro = mysql_fetch_row($result);        
		$num_presu1 = $registro[0]+1;
	}
	
	$nfilas2 = 0;
	$consulta2 = "SELECT n_presupuesto FROM presupuesto_vta_no_cliente where cod_talonario = '$codigo_tal'"; 				// verifica si se han hecho facturas a un NO cliente
	$result2 = mysql_query($consulta2);            					
	$nfilas2 = mysql_num_rows ($result2);          					
	if ($nfilas2 > 0){												// si existen facturas obtiene el mayor
		$consulta2 = "SELECT max(n_presupuesto) FROM presupuesto_vta_no_cliente where cod_talonario = '$codigo_tal'"; 
		$result2 = mysql_query($consulta2);            
		$registro2 = mysql_fetch_row($result2);        
		$num_presu2 = $registro2[0]+1;
	}
	
	if($num_presu2 > $num_presupuesto || $num_presu1 > $num_presupuesto){
		if($num_presu2 > $num_presu1){
			$num_presupuesto = $num_presu2;
		}else{
			$num_presupuesto = $num_presu1;
		}
	}
	...
	$num_presupuesto = buscar_numero($codigo_tal,$numero_tal, $num_presupuesto,$fecha);
	*/
	...
	//***********************************************************************************************************//	
			...
			if ($cod_cliente != "no_cliente"){ 
				...
				$consulta = "call alta_presupuesto_vta($num_presupuesto,$fecha,'$lugar','$hora',$cod_cliente,$cod_zona,$cod_localidad,$cod_prov,$cod_pais,$numero_rem,'$codigo_tal',$numero_tal,$categoria,$vendedor,$repartidor,'$obs',$tasa_iva,$tasa_perc_iva,$tasa_img_bruto,$monto_imp_int,'$usuario_presu',$forma_pago)";
				//echo $consulta;
			}
			//---------------------SI NO ES CLIENTE---------------------------------//
			if ($cod_cliente == "no_cliente"){
				$consulta = "call alta_presupuesto_vta_no_cliente($num_presupuesto,$fecha,'$lugar','$hora','$obs','$razon',$zona,'$dir','$localidad','$provincia',$cond_iva,'$cuit',$categoria,$vendedor,$repartidor,'$codigo_tal',$numero_tal,$numero_rem,$tasa_iva,$monto_imp_int,$tasa_perc_iva,$tasa_img_bruto,'$usuario_presu',$forma_pago)";
			}
			if($result = mysql_query($consulta)){        // hace la consulta
				...
				*/
				...
				//                            DATOS DEL DETALLE
				//***********************************************************************************************************//	
				$consulta = "SELECT * FROM presupuesto_vta_tmp where usuario = '$usuario_presu' ORDER BY linea";
				$result = mysql_query($consulta);            // hace la consulta
				$registro = mysql_fetch_row($result);        // toma el registro
				...
				do{
					...
					//---------------------SI ES CLIENTE---------------------------------//
					if ($cod_cliente != "no_cliente"){ 
						$consulta3 = "call alta_presupuesto_vta_detalle($num_presupuesto,$cod_producto,$cod_variedad,$cod_marca,$cod_grupo,$cantidad,$precio,$bonificacion,'$codigo_tal',$numero_tal,$tasa_iva)"; 
					} 
					//---------------------SI NO ES CLIENTE------------------------------//
					if ($cod_cliente == "no_cliente"){ 
						$consulta3 = "call alta_presupuesto_vta_detalle_no_cliente($num_presupuesto,$cod_producto,$cod_variedad,$cod_marca,$cod_grupo,$cantidad,$precio,$bonificacion,'$codigo_tal',$numero_tal,$tasa_iva)"; 				
					}
			   ...
			   //***********************************************************************************************************//	
			   $consulta4 = "call vaciar_tabla_presupuesto_vta_tmp('$usuario_presu')";
			   $result4 = mysql_query($consulta4);            // hace la consulta
			   
			  //----------------------------------------------------------------------------------------------------------------//				   
			   $len_num_presupuesto=strlen($num_presupuesto); 						// completo el numero de factura con ceros
			   while ($len_num_presupuesto < 8){								// completo el numero de la factura con ceros
					$ceros.="0";
					$len_num_presupuesto++;
			   }
			   $num_presupuesto=$ceros.$num_presupuesto;
			  ...
			  //if($codigo_tal == 'X'){
				  ...
				  include("imprimir_presupuesto_pdf.php");	 							// Incluye la pagina para imprimir las facturas
			  //}
			  /*
			  else{ 
...
}else{ // nuevo presupuesto - obtiene los parametros **************************************************************************************
	...
	$modulo="presupuestos";
	$plantilla = "alta_presupuesto_vta.tpl";
