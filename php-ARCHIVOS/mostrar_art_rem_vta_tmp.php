<?
	...
	$pag_consulta = "SELECT * FROM remito_vta_tmp where usuario = '$usuario_rem' ORDER BY linea"; 									// consulta sql
	//---------------------Paginacion----------------------------------------------------------------------------------------//
		...
		while($registro = mysql_fetch_array($pag_result)){ 	// obtengo los resultados 
					...
					echo "<tr onMouseOver=color_seleccion(this,'E0E0E0'); onMouseOut=color_defecto(this,'EAEAEA'); bgcolor='#EAEAEA'>"; //efecto ded color en las filas
						...
						echo "<td $clase align='center'>";
							...
							<input type='hidden' id='<? echo 11..$fila; ?>' name='<? echo $fila; ?>' onKeyPress='return solo_entero(event)'  onBlur="actualizar_cant_art_rem_vta_blur(<? echo 11..$fila; ?>)" onKeyUp='actualizar_cant_art_rem_vta(event, <? echo 11..$fila; ?>)' size='5' maxlength='5' class='caja_fila' value='<? echo $cantidad ?>'>     
							<?
						echo"</td>"; 
						echo"<td $clase align='center'>";  
								if ($stock < $cantidad){
						...
						echo"</td>";						 
						echo"<td $clase align='center'>"; 
							?> <img  class='iconos' src='../imagenes/eliminar.png' border='0' title="Eliminar" onClick="javascript: eliminar_art_rem_vta_tmp(' <? echo $fila ?>')"> <? 
						echo"</td>"; 		
					echo"</tr>";
						...
						//**************************************** TABLA DE TOTALES ***************************************************//
							echo "<table width='50%' align='right' border='0' cellspacing='1' cellpadding='0' bordercolor='#CCCCCC' bgcolor='#FBFBFB'>";
								...
								echo"<tr>";
									...
									echo"<td align='right'>$total_importe <input type='hidden'  id='sub_total'  name='sub_total' value='$total_importe'> </td>";
