<?
	...
	class PDF extends FPDF {
		...
		function Footer($observacion, $codigo_tal, $iva_importe_imprimir, $tasa_iva, $neto_gravado, $importe_iva, $no_gravado, $iibb, $total_presupuesto){
				...
				foreach ($iva_importe_imprimir as $tasa_iva => $neto_gravado) {
						...
						if($codigo_tal == "A"){
							...
							$total_presupuesto = $neto_gravado + ($neto_gravado * $tasa_iva/100);
							$importe_iva = number_format($importe_iva,2,'.','');
						}else{
						 	$importe_iva = "";
						 	$total_presupuesto = $neto_gravado;
						}
						
						$total_presupuesto = number_format($total_presupuesto,2,'.','');
						$suma_total_presupuesto = $suma_total_presupuesto + $total_presupuesto;
						...
						$this->SetX(170);
						$this->Cell(0,3,$total_presupuesto,0,1);		
				}
				
				$this->Ln(4);
				$this->SetFont('Arial','',10);										//Arial negrita 10
				$this->SetX(-30);
				$this->Cell(1,3 ,$suma_total_presupuesto,0,0,'R');
	...
	$codigo_tal = 'PRESUPUESTO Nro: '.$num_sucursal.'-'.$num_presupuesto;									// Tipo de talonario
	$nombre_pdf="$num_sucursal-$num_presupuesto";   	// Numero de Factura   
	
	$num_sucursal="N: $num_sucursal - $num_presupuesto";   	// Numero de Factura   
	...
	// footer
	$pdf->Footer($observacion, $codigo_tal, $iva_importe_imprimir, $tasa_iva, $neto_gravado, $importe_iva, $no_gravado, $iibb, $total_presupuesto);
	
	//---------------------- creo el archivo PDF------------------------------------------------//
	$pdf->Output('pdf/presupuestos/'.$nombre_pdf.'.pdf','F');   // guarda en el server
	...
	//________________________________________________________________________________________________________________________
	 ...
	 $ipp->setData('pdf/presupuestos/'.$nombre_pdf.'.pdf');				//  Toma la cadena para imprimir
