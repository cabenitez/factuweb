...
if($_POST["pag_consulta_ajax"]){		
	$pag_consulta = $pag_consulta_ajax;
}

//----------------------- Establece unn patron para la consulta sql-------------------------------------------//
$pag_consulta = ereg_replace("@@","'",$pag_consulta);
...
if ($total_paginas > 1){ 						// si la cantidad de registros es menor a la cantidad q se debe mostrar
	...
	$pag_navegacion ="<br>"."<div class='$estilo_pag_nav'>"; 
		...
		if($sig < $pag_inicio+$pag_tamano){
				$pag_navegacion .=" "."<span  class='botones' onClick=$comdbl paginar2('$pag_origen','$pag_consulta','$sig','$nombre')$comdbl> >> </span>";	
		}else{
				$pag_navegacion .=" "."<span  class='botones'> >> </span>";																					
		}
	
		$pag_navegacion .=" "."<span  class='botones' onClick=$comdbl paginar2('$pag_origen','$pag_consulta','$pag_inicio','$nombre')$comdbl> >>| </span>";	// ULTIMA PAGINA
