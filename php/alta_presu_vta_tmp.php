...
if($codigo_art){
	...
	$consulta = "SELECT count(linea) FROM presupuesto_vta_tmp where usuario = '$usuario_rem'";		// obtiene la cantidad de articulos
	$result = mysql_query($consulta);            
	$registro = mysql_fetch_row($result);        
	$num_iter = $registro[0];		

	$consulta = "SELECT * FROM presupuesto_vta_tmp where usuario = '$usuario_rem' and cod_prod = $codigo_art and bonificacion = 100";		// obtiene la cantidad de articulos
	$result = mysql_query($consulta);            
	...
	if ($nfilas > 0){     						 		// si ya existe el articulo
		...
		if($num_iter < $max_iter){																	// verifica que las lineas sean menor al max	
			...
			$consulta = "call alta_presupuesto_vta_tmp('$usuario_rem', $codigo_art,'$desc_art',$cant_art,$precio_art,$bonif_art,$importe_art,$linea,$tasa_iva)"; // llama al procedimiento almacecnado
			$result = mysql_query($consulta);        
			//echo $consulta;
		}else{
