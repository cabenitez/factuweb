<?
		...
		if($nombre == "TODOS"){//="select * from variedad";
			$pag_consulta ="SELECT grupo.cod_grupo, grupo.descripcion, marca.cod_marca, marca.descripcion, variedad.cod_variedad, variedad.descripcion FROM variedad INNER JOIN (marca INNER JOIN grupo ON grupo.cod_grupo = marca.cod_grupo) ON marca.cod_marca = variedad.cod_marca  and marca.cod_grupo = variedad.cod_grupo ORDER BY grupo.cod_grupo, marca.cod_marca, variedad.cod_variedad";
		}else{
			$consulta = "SELECT grupo.cod_grupo, grupo.descripcion, marca.cod_marca, marca.descripcion, variedad.cod_variedad, variedad.descripcion FROM variedad INNER JOIN (marca INNER JOIN grupo ON grupo.cod_grupo = marca.cod_grupo) ON marca.cod_marca = variedad.cod_marca  and marca.cod_grupo = variedad.cod_grupo";
     		$consulta2 = " where ";
			include ("cascada_variedad.php");
			$pag_consulta = $consulta . " " . "  ORDER BY grupo.cod_grupo, marca.cod_marca, variedad.cod_variedad";
		}
		//echo $pag_consulta;
	//---------------------Paginacion----------------------------------------------------------------------------------------//
...
if($pag_filas > 0){
	...
	echo "<table width='100%'  border='0'cellspacing='1' cellpadding='0'>";
      	echo "<tr class='top'>"; //fila
        	...
        	echo "<td width='25%'><div align='center' class='seccion'>Descripcion</div></td>";
			echo "<td width='25%'><div align='center' class='seccion'>Marca</div></td>";
			echo "<td width='25%'><div align='center' class='seccion'>Grupo</div></td>";
		...
		while($registro = mysql_fetch_array($pag_result)){ 	// obtengo los resultados 
				...
				echo "<tr onMouseOver=color_seleccion(this,'E0E0E0'); onMouseOut=color_defecto(this,'EAEAEA'); bgcolor='#EAEAEA'>"; //efecto ded color en las filas
					...
					echo"</td>";
					echo "<td $clase align='left'>";
							echo $espacio_izq.$cod_marca."-".$marca;     
					echo"</td>"; 
					echo "<td $clase align='left'>";
							echo $espacio_izq.$cod_grupo."-".$grupo;     
					echo"</td>"; 
					
					$consulta_producto =  "select cod_prod from producto where cod_variedad = $codigo";	 // consulta en la base de datos si ya no se ha registrado el stock inicial
					$result_producto = mysql_query($consulta_producto);            // hace la consulta
					$nfilas_producto = mysql_num_rows ($result_producto);          //indica la cantidad de resultados

					echo"<td $clase align='center'>";	
							if ( $nfilas_producto == 0 ){     						 // si existe el usuario inicia la sesion
									echo "<img  class='iconos' src='../imagenes/modificar.png' border='0' alt='$codigo,$cod_marca,$cod_grupo' onClick='javascript: modificar_variedad($codigo,$cod_marca,$cod_grupo)'>"; 
							}else{
