<?
		...
		if($nombre == "TODOS"){
			$pag_consulta = "SELECT * FROM perc_iva ORDER BY perc_iva.nombre"; 									// consulta sql
		}else{
			$pag_consulta = "SELECT * FROM perc_iva where perc_iva.nombre LIKE '%$nombre%' order by perc_iva.nombre"; 	// consulta sql
		}
	//---------------------Paginacion----------------------------------------------------------------------------------------//
		...
		while($registro = mysql_fetch_array($pag_result)){ 	// obtengo los resultados 
				...
				echo "<tr onMouseOver=color_seleccion(this,'E0E0E0'); onMouseOut=color_defecto(this,'EAEAEA'); bgcolor='#EAEAEA'>"; //efecto ded color en las filas
					...
					echo"<td align='center'>";	
							echo "<img  class='iconos' src='../imagenes/modificar.png' border='0' title='Modificar' onClick='javascript: modificar_pi($codigo)'>"; 
					echo"</td>"; 
					echo"<td align='center'>"; 
							echo "<img class='iconos' src='../imagenes/eliminar.png' border='0' title='Eliminar' onClick='javascript: eliminar_pi($codigo)'>"; //onClick='javascript: abreventanaprecio($codigo)'
					echo"</td>"; 		
				echo"</tr>";
		}
	echo "</table>";   
