<?
		...
		if($nombre == "TODOS"){
			$pag_consulta = "SELECT * FROM imp_interno ORDER BY imp_interno.nombre"; 									// consulta sql
		}else{
			$pag_consulta = "SELECT * FROM imp_interno where imp_interno.nombre LIKE '%$nombre%' order by imp_interno.nombre"; 	// consulta sql
		}
	//---------------------Paginacion----------------------------------------------------------------------------------------//
...
if($pag_filas > 0){
	...
	echo "<table width='100%'  border='0'cellspacing='1' cellpadding='0'>";
      	echo "<tr class='top'>"; //fila
        	...
        	echo "<td width='36%'><div align='center' class='seccion'>Importe</div></td>";
		...
		while($registro = mysql_fetch_array($pag_result)){ 	// obtengo los resultados 
				...
				echo "<tr onMouseOver=color_seleccion(this,'E0E0E0'); onMouseOut=color_defecto(this,'EAEAEA'); bgcolor='#EAEAEA'>"; //efecto ded color en las filas
					...
					echo"<td align='center'>";	
							echo "<img  class='iconos' src='../imagenes/modificar.png' border='0' title='Modificar' onClick='javascript: modificar_ii($codigo)'>"; 
					echo"</td>"; 
					echo"<td align='center'>"; 
							echo "<img class='iconos' src='../imagenes/eliminar.png' border='0' title='Eliminar' onClick='javascript: eliminar_ii($codigo)'>"; //onClick='javascript: abreventanaprecio($codigo)'
					echo"</td>"; 		
				echo"</tr>";
		}
	echo "</table>";   
