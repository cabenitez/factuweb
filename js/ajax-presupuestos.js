...
//----------------------------------FACTURA VENTA----------------------------------------------------//
///////////////////////////////////////////////////////////////////////////////////////////////////////
function pasar_foco_presu_vta_5(e){
...
/*
function ZZZZpasar_foco_fac_vta_11(e){ //caja lugar
	tecla = (document.all) ? e.keyCode : e.which; // 2
	if ( tecla==13 ){
...
function ZZZZpasar_foco_fac_vta_12(e){ //caja hora
	tecla = (document.all) ? e.keyCode : e.which; // 2
	if ( tecla==13 ){
...
function ZZZZpasar_foco_fac_vta_14(e){
	tecla = (document.all) ? e.keyCode : e.which; // 2
	if ( tecla==13 ){
...
function ZZZZpasar_foco_fac_vta_15(e){ //caja hora
	tecla = (document.all) ? e.keyCode : e.which; // 2
	if ( tecla==13 ){
...
*/
//-------------------------------------------------------------------------------------------------//
function buscar_num_presu_vta(){ 				// busca el numero de presupuesto
	...
	var divnumero_presu = document.getElementById("numero_presu");
	var txt_numero_presu = document.getElementById("oculto_numero_presu");

	var ajax3=nuevoAjax();										  // creo una instancia de ajax
			metodo="GET";												  // asigno las variables de proceso
			url="buscar_num_presu_vta.php?";
			...
			ajax3.onreadystatechange=function(){ 
				if (ajax3.readyState==4){
					...
					if (error  == 0){ //si encuentra el cliente buscado
						...
						var numero_presu = aux.getElementsByTagName('numero_presu').item(0).firstChild.data;
						...
						divnumero_presu.innerHTML = numero_suc+'-'+numero_presu+'-'+codigo_tal;
						txt_numero_presu.value = numero_presu;
					}else{
						// asignamos el valor de las variables del XML a los objetos
						div_numero_tal.innerHTML = '0000';
						txt_numero_tal.value = 'ERROR';
						txt_codigo_tal.value = 'ERROR';
						divnumero_presu.innerHTML = '00000000';
						txt_numero_presu.value = 'ERROR';
...
//--------------------------------------------------------------------------------------------------//
function listar_forma_pago_presu(){
...
function seleccionar_cliente_presu_vta(){		// abre el pop up para seleccionar en cliente
...
function buscar_cliente_presu_vta(){			// realiza la busqueda con XML para traer los datos del cliente
			var divMensaje=document.getElementById("mensaje");  // asigna los aobjetos a las variables
			var txtcodigo = document.getElementById("codigo");
			var ajax=nuevoAjax();										  // creo una instancia de ajax
			metodo="GET";												  // asigno las variables de proceso
			url="buscar_cliente_alta_presu_vta.php?";
			variables="codigo="+txtcodigo.value;
...
//--------------------------------------------------------------------------------------------------//
function borrar_cajas_presupuesto_vta(){		
	document.getElementById("mensaje").innerHTML=""; 
...
//--------------------------------------------------------------------------------------------------//
function buscar_cliente_alta_presu_vta(e){
	tecla = (document.all) ? e.keyCode : e.which; // 2
	var codigo=document.getElementById("codigo");
	if ( tecla==113 ){ 
		seleccionar_cliente_presu_vta();
	}
	if ( tecla==13 &&  codigo.value.length > 0 && codigo.value != "0" ){
		buscar_cliente_presu_vta();
	}
	if ( tecla==13 &&  codigo.value.length == 0){
		borrar_cajas_presupuesto_vta();
		vaciar_tabla_presu_vta_tmp();
		document.getElementById("razon").focus();
	}
}
function vaciar_tabla_presu_vta_tmp(){
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
    url="vaciar_tabla_presu_vta_tmp.php?";
	ajax.open(metodo, url, true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send(null);
}
...
//---------------------------------------------------------------------------------------------------------------------------------//
function ZZlistar_iva_cliente_fac_vta(codigo){					// crea el select con la cond de iva del cliente
	var contenedor=document.getElementById("cond_iva"); 
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
...
function ZZlistar_iva_no_cliente_fac_vta(codigo){
	var contenedor=document.getElementById("cond_iva"); 
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
...
//-----------------------------------------------------//
function ZZlistar_cat_cliente_fac(codigo){
	var contenedor=document.getElementById("categorias"); 
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
...
//------------------------------------------------------//
function ZZlistar_forma_pago_fac(codigo){
	var contenedor=document.getElementById("cond_pago"); 
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
    url="listar_forma_pago_fac_vta.php";
...
//------------------------------------------------------//
function ZZlistar_cat_no_cliente_fac(codigo){
	var contenedor=document.getElementById("categorias"); 
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
...
//-----------------------------------------------------//
function ZZlistar_zona_cliente_fac(codigo){
	var contenedor=document.getElementById("zonas"); 
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
    url="listar_zona_fac.php";
...
function ZZlistar_zona_no_cliente_fac_vta(zona){
	var contenedor=document.getElementById("zonas"); 
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
...
//------------------------------------------------------------------------------//
function listar_art_remito_a_presupuesto_vta(num_remito,codigo_tal,numero_tal){
	var contenedor=document.getElementById("listado"); 
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
    url="listar_art_remito_a_presupuesto_vta.php";
...
//--------------------------------------------------------------------------------------------------//
function seleccionar_remito_presu_vta(){		// abre el pop up para seleccionar en cliente
	var win = window.open("buscar_remito_alta_presupuesto.php", "win",  "toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=500,height=550,top=100,left=200");
	
	//abrirVentanaFija('buscar_remito_alta_presupuesto.php', 500, 550, 'win', 'Buscar Remito');
}

function ZZstock_negativo(){
document.getElementById('mensaje').innerHTML="asdasds";
}

function buscar_remito_presu_vta(){			// realiza la busqueda con XML para traer los datos del cliente
			...
			var ajax=nuevoAjax();										  // creo una instancia de ajax
			metodo="GET";												  // asigno las variables de proceso
			url="buscar_remito_alta_presupuesto.php?";
			variables="codigo="+txtcodigo.value;
			ajax.open(metodo, url + variables, true); // envia los datos a la pagina php y esta la procesa
			ajax.onreadystatechange=function(){ 
				if (ajax.readyState==4){
							...
							if (error  == 0){ //si NO hay errores
										...
										if (tipo  == 'cliente'){ 
												...
												codigo_tal=document.getElementById("oculto_codigo_tal").value;
												numero_tal=document.getElementById("oculto_numero_tal").value;
												//alert(codigo_rem+'-'+codigo_tal+'-'+numero_tal);
												listar_art_remito_a_presupuesto_vta(codigo_rem,codigo_tal,numero_tal); // funcion para agregar los articulos de remito a faactura
												document.frm.lista_forma_pago.focus();
										}
...
//--------------------------------------------------------------------------------------------------//
function buscar_remito_alta_presu_vta(e){
	var divMensaje=document.getElementById("mensaje");  // asigna los aobjetos a las variables
	tecla = (document.all) ? e.keyCode : e.which; // 2
	var remito=document.getElementById("remito");
	if ( tecla==113 ){ //F2 pop up
		seleccionar_remito_presu_vta();
	}
	if ( tecla==13 &&  remito.value.length > 0 && remito.value != "0" ){
		buscar_remito_presu_vta(); // el boton y la caja de texto llaman al mismo funcion
	}
...
//--------------------------------------------------------------------------------------------------//
/*
function ZZlistar_loca_buscar_alta_fact(){
...
function ZZpasar_foco_fact_vta_bus_1(e){
	tecla = (document.all) ? e.keyCode : e.which; // 2
	if ( tecla==13 ){
...
function ZZpasar_foco_fact_vta_bus_2(e){
	tecla = (document.all) ? e.keyCode : e.which; // 2
	if ( tecla==13 ){
...
function ZZpasar_foco_fact_vta_bus_3(e){
	tecla = (document.all) ? e.keyCode : e.which; // 2
...
function ZZpasar_foco_fact_vta_bus_4(e){ //dia desde
		tecla = (document.all) ? e.keyCode : e.which; // 2
...
function ZZpasar_foco_fact_vta_bus_5(){   //mes desde
		if( document.frm.mes_desde.value.length == 2  ) {
			document.frm.ano_desde.focus();
			return 0;		 
	   }		
}
function ZZpasar_foco_fact_vta_bus_6(){   // año desde
		if( document.frm.ano_desde.value.length == 4  ) {
			document.frm.dia_hasta.focus();
			return 0;		 
	   }		
}
function ZZpasar_foco_fact_vta_bus_7(e){  // dia hasta
		tecla = (document.all) ? e.keyCode : e.which; // 2
...
function ZZpasar_foco_fact_vta_bus_8(){   //mes hasta
		if( document.frm.mes_hasta.value.length == 2  ) {
			document.frm.ano_hasta.focus();
			return 0;		 
	   }		
}
function ZZpasar_foco_fact_vta_bus_9(e){
	  tecla = (document.all) ? e.keyCode : e.which; // 2
...
//--------------------------------------------------------------------------------------------//
function buscar_remito_alta_presu_proceso(){
	...
	url="buscar_remito_alta_presu_proceso.php?";
	var cant_objetos = document.frm.elements.length;
	for (i=0; i < cant_objetos; i++){		//deshabilito todos los elementos
			document.frm.elements[i].disabled=true;
	}
...
//-------------------------------------------------------------------------------------------------------------//
function buscar_remito_alta_presu_no_cliente_proceso(){
	...
	url="buscar_remito_alta_presu_no_cliente_proceso.php?";
	var cant_objetos = document.frm.elements.length;
	for (i=0; i < cant_objetos; i++){		//deshabilito todos los elementos
			document.frm.elements[i].disabled=true;
	}
...
//--------------------------------------------------------------------------------------------------//
function ZZeliminar_art_fac_vta_tmp(fila){
		var divMensaje=document.getElementById("mensaje"); 
	...
	ajax.onreadystatechange=function (){ 
			if (ajax.readyState==4){
				mostrar_art_presu_vta_tmp()
			} // fin de if (ajax.readyState==4)
		} // fin de funcion()
}
//--------------------------------------------------------------------------------------------------//
function mostrar_art_presu_vta_tmp(){
	var divlistado=document.getElementById("listado"); 
	...
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
    url="mostrar_art_presu_vta_tmp.php?";
...
//--------------------------------------------------------------------------------------------------//
function actualizar_cant_art_presu_vta(e,id_Text){
	tecla = (document.all) ? e.keyCode : e.which; // 2
	if ( tecla==13 ){
			...
			if(document.getElementById(id_Text).value != "0" && document.getElementById(id_Text).value.length > 0){ //&& document.getElementById(id_Text).value <= cant_max
					...
					metodo="POST";												  // asigno las variables de proceso
					url="modificar.php";
					variables="fila_art_presu_vta="+id_Text+"&cantidad="+cant;
					//alert(variables);
					ajax.open(metodo, url, true);
					ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
					ajax.send(variables);
...
function actualizar_cant_art_presu_vta_blur(id_Text){
			cant_max=document.getElementById(id_Text).name;
			if(document.getElementById(id_Text).value != "0" && document.getElementById(id_Text).value.length > 0){ //&& document.getElementById(id_Text).value <= cant_max
					var cant=document.getElementById(id_Text).value;
					var ajax=nuevoAjax();										  // creo una instancia de ajax
...
//--------------------------------------------------------------------------------------------------//
function actualizar_bonif_art_presu_vta(e,id_Text){
	tecla = (document.all) ? e.keyCode : e.which; // 2
	if ( tecla==13 ){
			//cant_max=document.getElementById(id_Text).name;
			if(document.getElementById(id_Text).value.length != "0" && document.getElementById(id_Text).value <= 100){ //&& document.getElementById(id_Text).value <= cant_max
					...
					metodo="POST";												  // asigno las variables de proceso
					url="modificar.php";
					variables="fila_bonif_art_presu_vta="+id_Text+"&bonif="+bonif;
					//alert(variables);
					ajax.open(metodo, url, true);
					ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
					ajax.send(variables);
...
function actualizar_bonif_art_presu_vta_blur(id_Text){
			//cant_max=document.getElementById(id_Text).name;
			if(document.getElementById(id_Text).value.length > 0 && document.getElementById(id_Text).value <= 100){ //&& document.getElementById(id_Text).value <= cant_max
					var bonif=document.getElementById(id_Text).value;
					var ajax=nuevoAjax();										  // creo una instancia de ajax
...
function agregar_articulo_presu_vta(){
...
//--------------------------------------------------------------------------------------------------//
function calcular_importe_presu(e){
	tecla = (document.all) ? e.keyCode : e.which; // 2
	var divMensaje = document.getElementById("mensaje_art");  // asigna los aobjetos a las variables
...
function registrar_presupuesto_vta(){
	var divMensaje = document.getElementById("mensaje");  				// asigna los aobjetos a las variables
	...
	var txtnumero_presu = document.getElementById("oculto_numero_presu");  	// asigna los aobjetos a las variables
	...
	if(txtnumero_tal.value != " " && txtcodigo_tal.value != " " && txtnumero_presu.value != " "){
		if(razon != ""){
			if(dir != ""){
				if(localidad != ""){
					if(zona != ""){
							...
							if(repartidor != ""){
								if(categoria != ""){
										if(forma_pago != ""){ 													// finaliza la validacion de la cabezera
													var ajax=nuevoAjax();										// creo una instancia de ajax
													metodo="POST";												// asigno las variables de proceso
													url="consultar_art_presu_vta_tmp.php";						// consulto si existen articulos en la tabla temporal
													ajax.open(metodo, url, true);
													ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
													ajax.send(null);
													ajax.onreadystatechange=function (){ 
																					...
																					if (ajax2.readyState==4){
																						...
																						if(ajax2.responseText != 'ook'){	
																							...
																							//document.getElementById("codigo").focus()
																							 ...
																							 if(document.frm.remito.value.length > 0 && document.frm.remito.value != "0"){
																									divMensaje.innerHTML = "ERROR: el presupuesto no pudo registrarse, verifique el Nº de Remito"; //=  ajax2.responseText;
																							/*
																							}else{
																								if(ajax2.responseText == 'existe_carga'){	
																									divMensaje.innerHTML  = "ERROR: el  presupuesto no pudo registrarse, La carga "+repartidor +" ya ha sido finalizada"; //= ajax2.responseText;
																								}
																							*/
																							}
																						}
		...
		divMensaje.innerHTML="ERROR: Verifique de que exista un Talonario de Presupuesto";
	 }
}


//////////////////////////////////////////////////////////////////////////////////////////////////////
//--------------------------------- DESCARGAR PRESUPUESTOS DEL SERVIDOR ----------------------------//
//////////////////////////////////////////////////////////////////////////////////////////////////////

function listar_presupuestos_servidor(){
	var contenedor=document.getElementById("listado"); 
	contenedor.innerHTML= '<img src="../imagenes/progress10.gif">'; // width="30" height="30"
	url="descargar_presupuesto_vta.php";			
	var ajax=nuevoAjax();										  // creo una instancia de ajax
	metodo="POST";												  // asigno las variables de proceso
	variables="listar=1";
	ajax.open(metodo, url, true);
...
function descargar_presupuesto_servidor(archivo){
...
/*
function ZZmarcar_pedido_como_facturado(){
...
//------------------------------------REGISTRAR FACTURA VENTA-----------------------------------------//
function ZZregistrar_nota_credito(){
	var divMensaje = document.getElementById("mensaje");  // asigna los aobjetos a las variables
